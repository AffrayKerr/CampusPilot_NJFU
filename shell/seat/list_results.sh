...
results=$(shell_db_query "SELECT * FROM seat_results WHERE user_id = ? ORDER BY created_at DESC LIMIT ?" "$user_id" "$limit")

shell_response_json true "Seat results retrieved" "$results"
