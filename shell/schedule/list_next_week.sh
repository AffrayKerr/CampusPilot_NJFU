...
if ! result_json="$("$AUTH_PYTHON" "$SCRIPT_DIR/schedule_scraper.py" list_next_week "$user_id" 2>&1)"; then
  shell_log_write ERROR schedule "list next week failed" "user_id=$user_id result=$result_json" "$user_id"
  if [[ -z "$result_json" || "$result_json" != *'"success"'* ]]; then
    result_json="$(shell_response_json false "list next week failed" null)"
  fi
  printf '%s\n' "$result_json"
  exit 1
fi
